...
NSString *const ZKDownloadNetworkSpeedNotificationKey = @"ZKDownloadNetworkSpeedNotificationKey";
NSString *const ZKUploadNetworkSpeedNotificationKey   = @"ZKUploadNetworkSpeedNotificationKey";
NSString *const ZKNetworkSpeedNotificationKey         = @"ZKNetworkSpeedNotificationKey";

@interface ZKNetworkSpeedMonitor () {
    // 总网速
    uint32_t _iBytes;
    uint32_t _oBytes;
    uint32_t _allFlow;
...
@implementation ZKNetworkSpeedMonitor
...
- (void)checkNetworkSpeed {
    ...
    if (_iBytes != 0) {
        ...
        [[NSNotificationCenter defaultCenter] postNotificationName:ZKDownloadNetworkSpeedNotificationKey object:nil userInfo:@{ZKNetworkSpeedNotificationKey:_downloadNetworkSpeed}];
//        NSLog(@"downloadNetworkSpeed : %@",_downloadNetworkSpeed);
    ...
    if (_oBytes != 0) {
        ...
        [[NSNotificationCenter defaultCenter] postNotificationName:ZKUploadNetworkSpeedNotificationKey object:nil userInfo:@{ZKNetworkSpeedNotificationKey:_uploadNetworkSpeed}];
